...
class TestMetadata(testtools.TestCase):
    ...
    def test_replace(self):
        ...
        self.metadata._replace = mock.Mock(side_effect=side_effect_func)
        path, body = self.metadata.replace(self.instance_uuid,
                                           self.metadata_key,
                                           self.metadata_value)
        self.assertEqual('/instances/%s/metadata/%s' %
                         (self.instance_uuid, self.metadata_key), path)
        self.assertEqual(replace_body, body)
    ...
    def test_parse_value_valid_json_in(self):
        ...
        newvalue = self.metadata._parse_value(ser_value)
        self.assertEqual(value, newvalue)

    def test_parse_value_string_in(self):
        value = 'this is a string'
        newvalue = self.metadata._parse_value(value)
        self.assertEqual(value, newvalue)
